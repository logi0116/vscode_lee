...
public class SignUp extends JFrame { // 메인 클래스는 반드시 public이어야 하며, JFrame을 상속받아 창 역할을 합니다.
    ...
    private void initUI() {
        ...
        // BorderLayout은 NORTH(상단), SOUTH(하단), EAST(우측), WEST(좌측), CENTER(중앙)의 5개 영역을 가집니다.
   ...
   private void refreshTable() {
    ...
    private void showAddDialog() {
        ...
        panel.add(new JLabel("이메일 :"));     panel.add(emailField);
...
Member oldMember = memberService.getMembers().get(selectedRow);
        ...
        if (result == JOptionPane.OK_OPTION) { // 사용자가 '확인' 버튼을 클릭했을 경우
            ...
            Member updatedMember = new Member(updatedName, updatedEmail, updatedPassword, oldMember.getRegistrationDate()); // 새 Member 객체 생성 (가입일은 유지)
...
Member memberToDelete = memberService.getMembers().get(selectedRow);        String memberName = memberToDelete.getName(); // 확인 메시지에 사용할 회원 이름
        ...
        if (confirmResult == JOptionPane.YES_OPTION) { // 사용자가 '예' 버튼을 클릭했을 경우
           ...
           memberService.saveChanges();         // 변경된 회원 목록을 파일에 저장
            ...
            JOptionPane.showMessageDialog(this, memberName + " 회원이 성공적으로 삭제되었습니다.", "알림", JOptionPane.INFORMATION_MESSAGE);
